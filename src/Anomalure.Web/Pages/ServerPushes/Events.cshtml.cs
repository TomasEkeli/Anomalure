...
) : PageModel
{
			...
			async (stream, cancellationToken) =>
			{
				try
				{
					...
					while (!cancellationToken.IsCancellationRequested)
					{
						...
						if (isHtmx)
						{
							await WriteHtmlEvent(writer, data: data);
						}
						else
						{
							await WriteTextEvent(writer, data: data);
						}
	...
	static async Task WriteHtmlEvent(StreamWriter writer, string data) =>
		await WriteEvent(writer, data: "<div>" + data + "</div>");

	static async Task WriteTextEvent(StreamWriter writer, string data) =>
		await WriteEvent(writer, data: data);
